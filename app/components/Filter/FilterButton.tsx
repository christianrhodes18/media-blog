...
const FilterButton: React.FC<FilterButtonProps> = ({
  ...
  const createQueryString = (type: string, option: string): string => {
    ...
    if (currentTagFilters && type === "tag") {
      // if currentTagFilters exists and type is tag
      if (currentTagFilters.includes(option)) {
        // if option is already selected, remove it from currentTagFilters
        const newTagFilters = currentTagFilters.filter(
          (item) => item !== option
        ); // remove option from currentTagFilters
    ...
    } else if (currentAestheticFilters && type === "aesthetic") {
      // if currentAestheticFilters exists and type is aesthetic
      if (currentAestheticFilters.includes(option)) {
        // if option is already selected, remove it from currentAestheticFilters
        const newAestheticFilters = currentAestheticFilters.filter(
          (item) => item !== option
        ); // remove option from currentAestheticFilters
  ...
  return (
    <div className="h-6 cardBGLightDark rounded-3xl px-4">
      <button
        onClick={() => setActive(!active)}
        className="flex flex-row justify-center items-center gap-2"
      >
        <p className="body2 p_no_padding">{type}</p>
        {!active && (
          <Image
            src="/icons/chevron_down.svg"
            alt={`filter by ${type} dropdown`}
            width="12"
            height="12"
          />
        )}
          ...
          <div className="absolute cardBGLightDark rounded-md m-2 min-w-[6rem] z-10 flex flex-col">
            {dropdownData.map((option) => {
              return (
                ...
                <Link
                  key={option}
                  href={createQueryString(type, option)}
                  onClick={() => updateSelectedData(selectedData, option)}
                  className={`text-left body2 px-2 h-8 ${
                    selectedData.includes(option) ? "bg-[#A3C0FB]" : ""
                  }`}
                >
                  {option}{" "}
                </Link>
