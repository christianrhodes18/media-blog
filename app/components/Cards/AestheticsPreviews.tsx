      ...
      {Object.keys(data).map((aestheticName, index) => {
        ...
        return (
          <Link key={index} href={`/about#${aestheticName}`} passHref>
            <div
              key={aestheticName}
              className={`min-w-[352px] min-h-[242px] flex rounded-lg bg-[url('/about/${aesthetic.image}')] text-darkText hover:text-opacity-10 hover:scale-110 transition-all bg-cover`}
              style={{ backgroundImage: `url('/about/${aesthetic.image}'` }}
            >
