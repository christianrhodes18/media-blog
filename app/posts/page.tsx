...
export default async function Posts({
  ...
  let selectedTagString: string = Array.isArray(searchParams.tag)
  ...
  let selectedAestheticString: string = Array.isArray(searchParams.aesthetic)
    ? searchParams.aesthetic.join(",")
    : searchParams.aesthetic || "all";
  ...
  const updateFilters = (filterType: string, newFilterOptions: string[]) => {
    ...
    if (filterType === "tag") {
      ...
      if (selectedTagString !== "all") {
        // tags already selected from URL
        selectedTagString = newFilterOptions.join(","); // update selectedTagString
        newFilterOptions.map((tag) => {
          // add new tags to selectedTagArray
          if (!selectedTagArray.includes(tag)) {
            selectedTagArray.push(tag);
          }
        });
      } else {
        // no tags selected from URL so it is "all"
        selectedTagString = newFilterOptions.join(","); // update selectedTagString
  ...
  const updatePosts = () => {
    ...
    if (selectedTagString !== "all" && selectedAestheticString !== "all") {
      filteredPosts = posts.filter(
        (post: { tags: string[]; aesthetics: string[] }) => {
          ...
          return (
            post.tags.some((tag: string) => selectedTagArray.includes(tag)) &&
            post.aesthetics.some((aesthetic: string) =>
              selectedAestheticArray.includes(aesthetic)
            )
          );
        }
      );
    ...
    if (selectedAestheticString !== "all") {
      filteredPosts = posts.filter((post: { aesthetics: string[] }) => {
        ...
        return post.aesthetics.some((aesthetic: string) =>
          selectedAestheticArray.includes(aesthetic)
        );
      });
    }
    ...
    <div className="w-[90%] md:w-[85%] flex flex-col max-w-5xl mx-auto sm:mt-12 mb-12 min-h-[65dvh]">
      ...
      <div className="mt-2 mb-4 flex flex-row gap-4">
        ...
        {selectedTagString !== "all" ? (
          <FilterButton type="tag" currentTagFilters={selectedTagArray} />
        ) : (
      ...
      {(selectedTagString !== "all" || selectedAestheticString !== "all") && (
        <div className="flex flex-col min-[450px]:flex-row gap-2 mb-4">
          ...
          {selectedTagString !== "all" && (
            <ClearFilterButton type="tag" appliedFilters={selectedTagArray} />
          )}
